...
*** Email with mu4e
- *mu4e* email client with three-account setup (personal, work, iCloud) via mu4e contexts
- *mbsync* (isync) for fast IMAP sync to local Maildir at =~/.mailbox=
- *msmtp* for outgoing mail, account selected automatically from the From address
- *mu4e-views* with xwidgets webkit for proper HTML rendering; press =v= to switch renderers
- *GPG-encrypted* credentials via =~/.authinfo.gpg= - shared by mbsync, msmtp, and Emacs auth-source
- *gnus-icalendar* for accepting/declining calendar invites directly in mu4e
- Custom bookmarks scoped to INBOX folders to avoid mailing list noise in unread counts
- Run =M-x mu4e= to open; =C-c C-c= to send; =C-c C-x= to switch context when composing

*** IRC with ERC and ZNC
- *ERC* (built-in) connects to a *ZNC bouncer* on the homelab over Tailscale (=znc.tailebe1e.ts.net:8080=)
...
- *GPG-encrypted* credentials via =~/.authinfo= - password never stored in config
- Run =M-x gn/erc-znc= to connect; auto-reconnect on disconnect
...
Email is not fully portable from this repo - you need to configure two files manually:
...
mu init --maildir=~/.mailbox \
  --my-address=you@domain1.com \
  --my-address=you@domain2.com \
  --my-address=you@domain3.com
mu index
#+end_src

Then run =M-x mu4e=. mu4e will keep the index current by running mbsync and re-indexing every 5 minutes automatically.
...
**** App passwords
Gmail requires an app-specific password (not your account password) when using IMAP with 2FA enabled. Generate one at [[https://myaccount.google.com/apppasswords][myaccount.google.com/apppasswords]] and store it in =~/.authinfo.gpg=.
