...
- *mbsync* (isync) for fast IMAP sync to local Maildir
- *msmtp* for outgoing mail, selected automatically by From address
- *GPG-encrypted* credentials via =~/.authinfo.gpg= — shared by mbsync, msmtp, and Emacs auth-source
- Run =M-x mu4e= to open; =C-c C-x= to switch context when composing
...
#+begin_src bash
mkdir -p ~/Mail
mbsync -c ~/.config/isync/config -a   # initial sync (may take a while)
...
- *Font*: Search for "Maple Mono" to change font family and size
