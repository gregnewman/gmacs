...
[[https://github.com/gregnewman/gmacs][View on GitHub]]
...
|------------------+----------------|
| macOS Tahoe      | Emacs 31.0.90  |

** Key Features

*** Modern LSP-based Development
- *Eglot integration* with tree-sitter for fast, accurate syntax parsing and Combobulate for intelligent code navigation
- *Python*: Ty (Astral's Rust-based type checker, 10-80x faster) or basedpyright, with ruff for linting and automatic formatting
...
- Hybrid project model: projects as headings with nested NEXT actions; agenda flattens to execution lists
- Per-area org files: =inbox.org=, =nanog.org=, =freelance.org=, =personal.org=, =finances.org=, =home.org=, =health.org=, =Studio.org=
- 5 capture templates: =t= (Todo→NEXT), =n= (Note), =k= (Knowledge Investment), =m= (Meeting), =r= (Weekly Review checklist)
- 4 agenda views: =d= Daily, =w= Weekly Review, =n= Next Actions by Context (@computer/@phone/@contact/@errands/@read/@agenda), =r= System Health
- Desktop notifications via =org-wild-notifier= (30, 10, and 5 min before events)
...
*** Email with notmuch
- *notmuch* tag-based email client with three-account setup (personal Gmail, work Gmail, iCloud)
- *mbsync* (isync) for fast IMAP sync to local Maildir at =~/.mailbox=
- *msmtp* for outgoing mail, account selected automatically from the From address via =--read-envelope-from=
- *GPG-encrypted* credentials via =~/.authinfo.gpg= - shared by mbsync, msmtp, and Emacs auth-source
- *gnus-icalendar* for accepting/declining calendar invites directly in notmuch
- Custom saved searches scoped to INBOX folders to avoid mailing list noise in unread counts
- Run =M-x notmuch= to open; =C-c C-c= to send; FCC disabled (Gmail/iCloud save sent mail server-side)

*** IRC with ERC and ZNC
- *ERC* (built-in) connects to a *ZNC bouncer* on the homelab over Tailscale (=znc.your-tailnet.ts.net:8080=)
- *Buffer playback* on reconnect - ZNC replays missed messages from =#emacs= and =#org-mode= on Libera.Chat
- *TLS* connection with self-signed cert trusted automatically (safe over Tailscale)
- *GPG-encrypted* credentials via =~/.authinfo.gpg= - password never stored in config
- Killing a channel buffer does *not* send PART — ZNC stays joined and buffers messages; use =/part= to actually leave
- Auto-connects on Emacs startup via =emacs-startup-hook=; run =M-x gn/erc-znc= to reconnect manually
...
- *Combobulate* for tree-sitter-aware structured editing and code manipulation
...
- =org-modern= for beautiful Org rendering (bullets, keywords, tables, blocks)
- =org-appear= for distraction-free markup (hides =*bold*= markers until cursor touches them)
- =olivetti= for centered, focused writing columns
- =mixed-pitch= for properly mixed variable/fixed-pitch fonts in Org
...
**** System Requirements
- Emacs 31.0 or later (recommended: Emacs Plus with native compilation)
- Git
- Homebrew (for macOS)
...
# Python development
brew install pyenv
uv tool install ty@latest  # Primary: Astral's fast Rust-based type checker
pip install basedpyright ruff  # Optional: fallback type checker
...
# Email
brew install notmuch isync msmtp
#+end_src
...
4. *Customize personal settings* in =gmacs.org=:
   ...
   - Font preferences (currently Maple Mono)
...
- Alert notifications
- Custom TODO keywords: NEXT → DONE, WAITING / SOMEDAY → CANCELLED
...
- *Python* - Full LSP with Ty (or basedpyright fallback), ruff linting, automatic formatting, pyenv integration. Toggle between LSP servers with =M-x my/switch-python-lsp=
...
**** GTD with Org-mode
- =C-c c= - Quick capture; then =t= todo, =n= note, =k= knowledge, =m= meeting, =r= weekly review
- =C-c a d= - Daily view (today's agenda + NEXT actions)
- =C-c a w= - Weekly review (inbox, waiting, projects, someday, 2-week calendar)
- =C-c a n= - Next actions filtered by context tag
- =C-c a r= - System health check (inbox size, all waiting/someday)
...
|-----------+--------------------------------------|
| C-c c     | Org capture                          |
| C-c c t   | Capture: Todo (→ NEXT in inbox)      |
| C-c c n   | Capture: Note                        |
| C-c c k   | Capture: Knowledge Investment        |
| C-c c m   | Capture: Meeting                     |
| C-c c r   | Capture: Weekly Review checklist     |
| C-c a d   | Agenda: Daily (today + NEXT actions) |
| C-c a w   | Agenda: Weekly Review                |
| C-c a n   | Agenda: Next Actions by context tag  |
| C-c a r   | Agenda: System Health                |
| C-c l     | Store link                           |
| C-c g     | Grab Mac link                        |
...
Email is not fully portable from this repo - you need to configure these files manually:

1. =~/.config/isync/config= - mbsync account config (IMAP servers, Maildir paths). A template matching this config's three-account structure is in the repo.
2. =~/.config/msmtp/config= - msmtp outgoing mail config (SMTP servers, one account per sender). Also in the repo.
3. =~/.authinfo.gpg= - GPG-encrypted credentials for all accounts (IMAP + SMTP). *Never commit this file.*

After creating both files:
#+begin_src bash
mkdir -p ~/.mailbox
mbsync -c ~/.config/isync/config -a   # initial sync (may take a while for large mailboxes)
notmuch new                            # index mail
#+end_src

Then run =M-x notmuch=. Use =U= (bound to =gn/notmuch-sync=) to sync and re-index from within Emacs.

**** Gmail notes
Gmail's IMAP throttles large initial syncs. If mbsync hits a bandwidth limit (=OVERQUOTA= error), wait a few minutes and re-run - it resumes where it left off. Use =caffeinate mbsync -c ~/.config/isync/config -a= on macOS to prevent sleep during the initial sync.

**** App passwords
Gmail and iCloud require an app-specific password (not your account password) when using IMAP with 2FA enabled.
- Gmail: generate at [[https://myaccount.google.com/apppasswords][myaccount.google.com/apppasswords]]
- iCloud: generate in Apple ID settings under App-Specific Passwords
Store passwords unquoted in =~/.authinfo.gpg=.
...
- *Font*: Search for "Iosevka NF" to change font family and size
- *Calendar location*: Search for "Charlotte, NC" to set your coordinates
- *Email accounts*: Search for "gregnewman.org" to update account names, addresses, and folder paths
...
2. Try toggling Python LSP server: =M-x my/switch-python-lsp=
3. Check the LSP stderr buffer: =M-x eglot-stderr-buffer=
4. Verify the server is running: =M-x eglot-events-buffer=
...
1. Install Maple Mono: download from [[https://github.com/subframe7536/maple-font][github.com/subframe7536/maple-font]]
