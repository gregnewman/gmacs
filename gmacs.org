...
#+END_SRC
** Todo Keywords
   GTD keyword sequences: =NEXT= (actionable) → =DONE=; =WAITING= (delegated/blocked, prompts for note) / =SOMEDAY= (parked) → =CANCELLED= (prompts for note). =WAITING= and =CANCELLED= prompt for a note on entry so there's an audit trail. Tasks timestamp when closed.
...
** Capture mode keybinding
   ...
   #+end_src
** Capture templates
   Five templates: =t= (Todo → inbox as NEXT, defaults to @computer), =n= (Note → inbox), =k= (Knowledge Investment → inbox), =m= (Meeting → inbox), =r= (Weekly Review → reviews.org as a dated checklist). All task captures land in inbox.org and get refiled during the weekly review.
     ...
     (with-eval-after-load 'org-agenda
       (setq org-agenda-custom-commands
             '(("d" "Daily"
               ...
               ("w" "Weekly Review"
                ((todo "NEXT" ((org-agenda-overriding-header "📥 Inbox — process to zero")
