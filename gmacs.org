...
#+BEGIN_SRC emacs-lisp
   (custom-set-faces
    ...
    '(default ((t (:family "Maple Mono NF" :height 130))))
    '(fixed-pitch ((t (:family "Maple Mono NF" :height 130))))

    ;; Current line number
    '(line-number-current-line ((t (:foreground "orange" :inherit line-number))))
    '(mode-line ((t (:family "Maple Mono NF" :weight Bold))))

    ;; Comments italic
    '(font-lock-function-name-face ((t (:family "Maple Mono NF":slant italic))))
    '(font-lock-variable-name-face ((t (:family "Maple Mono NF":weight bold)))))
  #+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; ;;   '(fixed-pitch ((t ( :family "Iosevka NF" :height 115))))
...
#+END_SRC
** Appearance Packages
   Packages that make Org documents more beautiful and enjoyable to write in.
   - =org-modern= renders bullets, TODO keywords, tags, tables, and block delimiters as clean unicode.
   - =org-appear= hides emphasis markers (=*bold*=, =/italic/=) until the cursor touches them.
   - =olivetti= centers text in a comfortable writing column.
   - =mixed-pitch= properly mixes variable-pitch (prose) and fixed-pitch (code/tables) fonts.
...
#+END_SRC
** Configuration
   Core Org-mode settings for GTD workflow. Agenda files are an explicit list of per-area files (inbox, nanog, freelance, personal, finances, home, health, studio). Context tags (@computer, @phone, @contact, @errands, @read, @agenda) are grouped as mutually exclusive so each NEXT action carries exactly one context. Refile targets scan up to 5 levels deep across all agenda files.
...
#+END_SRC
** Alerts
   Desktop notifications for Org agenda items. org-wild-notifier fires calendar-style reminders before scheduled items and deadlines — 30, 10, and 5 minutes before by default. Notifications are filtered to NEXT and WAITING keywords only. Uses the alert package with osx-notifier style for native macOS banners.
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package org-wild-notifier
    :straight (org-wild-notifier :type git :host github :repo "emacsorphanage/org-wild-notifier")
    :config
    (setq org-wild-notifier-alert-time '(30 10 5)
          org-wild-notifier-keyword-whitelist '("NEXT" "WAITING")
          org-wild-notifier-notification-title "Org")
    (org-wild-notifier-mode))
#+END_SRC
** Todo Keywords
   GTD keyword sequences: =INBOX= (captured, needs clarification) → =NEXT= (actionable) → =DONE=; =WAITING= (delegated/blocked, prompts for note) / =SOMEDAY= (parked) → =CANCELLED= (prompts for note). =WAITING= and =CANCELLED= prompt for a note on entry so there's an audit trail. Tasks timestamp when closed.

   #+BEGIN_SRC emacs-lisp
     ;; Keywords
     (setq org-todo-keywords
       '((sequence "INBOX(i)" "NEXT(n)" "|" "DONE(d)")
     ...
     (setq org-todo-keyword-faces
       '(("INBOX"     :foreground "cyan" :weight bold)
         ("NEXT"      :foreground "lime green" :weight bold)
...
** Capture mode keybinding
   ...
   #+end_src
** Capture templates
   Five templates: =t= (Todo → inbox as INBOX, no context yet), =n= (Note → inbox), =k= (Knowledge Investment → inbox), =m= (Meeting → inbox), =r= (Weekly Review → reviews.org as a dated checklist). All task captures land in inbox.org and get clarified during the weekly review — context tags and NEXT state are assigned then, not at capture time.

   Template expansion reference: =%?= cursor position, =%U= inactive timestamp, =%a= link to origin, =%t= active timestamp, =:clock-keep t= keeps running clock.
   ...
   (defun my/weekly-review-template ()
     (string-join
      '("* Weekly Review %t"
        "** Collect"
        "   - [ ] Process inbox.org to zero (refile or delete everything)"
        "   - [ ] Check paper notes — anything to capture digitally?"
        "** Clarify"
        "   - [ ] Review all WAITING items — any gone stale?"
        "   - [ ] Review all projects — does each have at least one NEXT action?"
        "** Organize"
        "   - [ ] Review SOMEDAY lists — anything to activate or delete?"
        "   - [ ] Check deadlines for next 2 weeks"
        "** Reflect"
        "   - [ ] Open weekly review agenda: C-c a w"
        "** Engage"
        "   - [ ] Write paper weekly plan")
      "\n"))
   #+end_src
     ...
     (setq org-capture-templates
           '(("t" "Todo" entry (file "~/Org/inbox.org")
             ...
             ("n" "Note" entry (file "~/Org/inbox.org")
              "* %? :NOTE:\n%U\n%a\n" :clock-keep t)
             ("k" "Knowledge Investment" entry (file "~/Org/inbox.org")
              "* %? :KI:\n%U\n%a\n" :clock-keep t)
...
** Agenda Views
   Custom agenda views for GTD workflow. =C-c a d= is the daily execution view. =C-c a w= drives the weekly review. =C-c a n= filters by context tag. =C-c a r= is a system health check.
     ...
     (with-eval-after-load 'org-agenda
       (setq org-agenda-custom-commands
             '(("d" "🌞 Daily"
               ...
               ("w" "☕ Weekly Review"
                ((todo "INBOX" ((org-agenda-overriding-header "📨 Inbox — process to zero")
                                (org-agenda-files '("~/Org/inbox.org"))))
                 (todo "WAITING"  ((org-agenda-overriding-header "🚧 Waiting For")))
                 (tags "PROJECT"  ((org-agenda-overriding-header "🔨 Projects — verify each has a NEXT")
               ...
               ("n" "🎯 Next Actions by Context"
                ((tags-todo "@computer" ((org-agenda-overriding-header "@computer")))
                 ...
                 (tags-todo "@contact"  ((org-agenda-overriding-header "@contact")))
                 (tags-todo "@errands"  ((org-agenda-overriding-header "@errands")))
                 (tags-todo "@read"     ((org-agenda-overriding-header "@read")))
                 (tags-todo "@agenda"   ((org-agenda-overriding-header "@agenda")))))

               ("r" "🚑 System Health"
                ((todo "" ((org-agenda-files '("~/Org/inbox.org"))
                           (org-agenda-overriding-header "Inbox")))
                 (todo "WAITING" ((org-agenda-overriding-header "All WAITING")))
                 (todo "SOMEDAY" ((org-agenda-overriding-header "All SOMEDAY"))))))))
   #+END_SRC
...
** Combobulate - Structured Editing
   Combobulate provides tree-sitter-aware structured editing and navigation for programming languages. Unlike traditional regexp-based approaches, Combobulate uses the concrete syntax tree maintained by tree-sitter to understand code structure, enabling intelligent navigation between functions, classes, statements, and other language constructs. It enhances standard Emacs structural movement commands (C-M-n/p/f/b/u/d) with tree-sitter awareness and adds powerful code manipulation features like dragging code blocks, cloning nodes, and progressive selection expansion. Works seamlessly with existing LSP, completion, and navigation tools.
...
#+END_SRC
* Email - notmuch
  notmuch is a fast, tag-based mail indexer backed by Xapian. The architecture is: mbsync
  fetches mail from IMAP to local Maildir (~/.mailbox), notmuch indexes it, and notmuch.el
  reads the index. Three accounts are configured via =notmuch-identities= and
  =notmuch-fcc-dirs=: gregnewman.org (personal Gmail with custom domain), NANOG (work Gmail),
  and Apple (iCloud). Outgoing mail is sent via msmtp, which reads credentials from
  =~/.authinfo.gpg= and selects the correct SMTP server based on the From address.
  Config lives at =~/.config/notmuch/default/config=. Run =M-x notmuch= to open;
  =C-c C-c= to send a composed message.
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package notmuch
      ...
      ;; ── Identities ────────────────────────────────────────────────────────
      ;; All three email addresses. notmuch-always-prompt-for-sender ensures the
      ;; From header is always confirmed when composing, preventing accidental
      ;; sends from the wrong account.
      (setq notmuch-identities
            '("Greg Newman <greg@gregnewman.org>"
              "Greg Newman <gnewman@nanog.org>"
              "Greg Newman <newman.greg@me.com>")
            notmuch-always-prompt-for-sender t)

      ;; ── Sent mail (FCC) ───────────────────────────────────────────────────
      ;; Disabled — Gmail automatically saves sent mail server-side, and
      ;; mbsync pulls it down on the next sync. No need for a local FCC copy.
      (setq notmuch-fcc-dirs nil)

      ;; ── Outgoing mail via msmtp ───────────────────────────────────────────
      ;; Identical to the previous mu4e setup - msmtp selects the correct SMTP
      ;; account via --read-envelope-from, so multi-account sending just works.
      ...
      (setq notmuch-search-oldest-first        nil
            ...
            notmuch-compose-dont-reply-to-self t
            ;; Show saved searches as a vertical list with key shortcuts visible.
      ...
      ;; ── Saved searches ────────────────────────────────────────────────────
      ;; Mirrors the previous mu4e bookmarks. folder: matches the Maildir path
      ;; under ~/.mailbox. The per-account INBOX searches replace mu4e's j-key
      ;; maildir shortcuts.
      (setq notmuch-saved-searches
            '((:name "Unread (INBOX)"
               :query "tag:unread AND NOT tag:trash AND (folder:gregnewman.org/INBOX OR folder:NANOG/INBOX OR folder:Apple/INBOX)"
               :key "u"
               :sort-order newest-first)
              (:name "Today"
               :query "date:today.. AND NOT tag:trash"
               :key "t"
               :sort-order newest-first)
              (:name "Last 7 days"
               :query "date:7d.. AND NOT tag:trash"
               :key "w"
               :sort-order newest-first)
              (:name "Flagged"
               ...
               :sort-order newest-first)
              (:name "gregnewman.org"
               :query "folder:gregnewman.org/INBOX"
               :key "g"
               :sort-order newest-first)
              (:name "NANOG"
               :query "folder:NANOG/INBOX"
               :key "n"
               :sort-order newest-first)
              (:name "Apple"
               :query "folder:Apple/INBOX"
               :key "a"
               :sort-order newest-first)
              (:name "emacs-orgmode"
               :query "folder:gregnewman.org/emacs-orgmode"
               :key "o"
               :sort-order newest-first)))

      ;; ── Hello screen key bindings ──────────────────────────────────────────
      ;; Explicit bindings since the :key property in saved searches is not
      ;; reliably activated in the hello widget display.
      (defun gn/notmuch-search-for (query)
        (notmuch-search query))

      (define-key notmuch-hello-mode-map "u"
        (lambda () (interactive) (notmuch-search "tag:unread AND NOT tag:trash AND (folder:gregnewman.org/INBOX OR folder:NANOG/INBOX OR folder:Apple/INBOX)")))
      (define-key notmuch-hello-mode-map "t"
        (lambda () (interactive) (notmuch-search "date:today.. AND NOT tag:trash")))
      (define-key notmuch-hello-mode-map "w"
        (lambda () (interactive) (notmuch-search "date:7d.. AND NOT tag:trash")))
      (define-key notmuch-hello-mode-map "f"
        (lambda () (interactive) (notmuch-search "tag:flagged AND NOT tag:trash")))
      (define-key notmuch-hello-mode-map "g"
        (lambda () (interactive) (notmuch-search "folder:gregnewman.org/INBOX")))
      (define-key notmuch-hello-mode-map "n"
        (lambda () (interactive) (notmuch-search "folder:NANOG/INBOX")))
      (define-key notmuch-hello-mode-map "a"
        (lambda () (interactive) (notmuch-search "folder:Apple/INBOX")))
      (define-key notmuch-hello-mode-map "o"
        (lambda () (interactive) (notmuch-search "folder:gregnewman.org/emacs-orgmode")))

      ;; ── Auto-sync ─────────────────────────────────────────────────────────
      ;; Poll every 5 minutes: fetch with mbsync then re-index with notmuch new.
      (defun gn/notmuch-sync ()
        "Fetch new mail with mbsync and re-index with notmuch."
        (interactive)
        (set-process-sentinel
         (start-process-shell-command
          "notmuch-sync" " *notmuch-sync*"
          "mbsync -c ~/.config/isync/config -a; notmuch new")
         (lambda (proc _event)
           (when (eq (process-status proc) 'exit)
             (message "notmuch: sync complete")))))

      (run-with-timer 60 60 #'gn/notmuch-sync)

      ;; ── Calendar invites ──────────────────────────────────────────────────
      ;; gnus-icalendar handles .ics attachments; goto-addr activates links.
      (require 'goto-addr)
...
* IRC - ERC via ZNC
  ERC is Emacs' built-in IRC client. It connects to a ZNC bouncer on the homelab at
  =znc.tailebe1e.ts.net:8080= (reachable over Tailscale). ZNC replays missed messages on
  reconnect. Credentials are read from =~/.authinfo.gpg= - never stored here.
...
#+BEGIN_SRC emacs-lisp
  (use-package erc
    ...
    ;; ZNC is on a private Tailscale network - trust its self-signed cert.
    (network-security-level 'low)
    :config
    ;; Don't send PART when killing a channel buffer. ZNC stays joined
    ;; and continues buffering messages until we reconnect. Use /part to
    ;; actually leave a channel.
    (advice-add 'erc-part-channel-on-kill :override #'ignore))

  (defun gn/erc-znc ()
    "Connect to ZNC bouncer at znc.tailebe1e.ts.net:8080 via Tailscale."
    (interactive)
    (erc-tls :server "znc.tailebe1e.ts.net"
             :port 8080
             :nick "gnewman"
             :full-name "Greg Newman"
             :password (auth-source-pick-first-password
                        :host "znc.tailebe1e.ts.net"
                        :user "gnewman"
                        :port "8080")))

  (add-hook 'emacs-startup-hook #'gn/erc-znc)
#+END_SRC
