...
* About
  ...
  and then call it.
* Packages
   `use-package` so that all packages are automatically installed if cloning this repo for another machine.
...
* Theme
   Configuration for my dark theme using solarized-gruvbox-dark because it is easy on my eyes and works great with lsp-ui
   #+BEGIN_SRC emacs-lisp
