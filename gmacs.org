...
#+BEGIN_SRC emacs-lisp
   (custom-set-faces
    ...
    '(default ((t (:family "Maple Mono" :height 130))))
    '(fixed-pitch ((t (:family "Maple Mono" :height 110))))

    ;; Current line number
    '(line-number-current-line ((t (:foreground "orange" :inherit line-number))))
    '(mode-line ((t (:family "Maple Mono" :weight Bold))))

    ;; Comments italic
    '(font-lock-function-name-face ((t (:family "Maple Mono":slant italic))))
    '(font-lock-variable-name-face ((t (:family "Maple Mono":weight bold)))))
  #+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; ;;   '(fixed-pitch ((t ( :family "Maple Mono" :height 115))))
...
#+END_SRC
* Email - mu4e
  mu4e is an Emacs mail client backed by mu, a fast maildir indexer. Mail is fetched locally via mbsync (~/.config/isync/config) and indexed by mu. Three accounts are configured via contexts: gregnewman.org (personal Gmail), NANOG (work Gmail), and Apple (iCloud). Switching contexts with =C-c C-x= when composing selects the correct From address and SMTP server. Mail syncs every 5 minutes automatically. Run =M-x mu4e= to open.
