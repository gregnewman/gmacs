...
#+BEGIN_SRC emacs-lisp
   (custom-set-faces
    ...
    '(default ((t (:family "Iosevka NF" :height 150))))
    '(fixed-pitch ((t (:family "Iosevka NF" :height 130))))

    ;; Current line number
    '(line-number-current-line ((t (:foreground "orange" :inherit line-number))))
    '(mode-line ((t (:family "Iosevka NF" :weight Bold))))

    ;; Comments italic
    '(font-lock-function-name-face ((t (:family "Iosevka NF":slant italic))))
    '(font-lock-variable-name-face ((t (:family "Iosevka NF":weight bold)))))
  #+END_SRC
...
#+END_SRC
* Email - mu4e
  mu4e is an Emacs mail client backed by =mu=, a fast Xapian-indexed maildir searcher. The
  architecture is: mbsync fetches mail from IMAP to local Maildir (~/.mailbox), mu indexes
  it, and mu4e reads the index. This means search is instantaneous - mu never touches the
  network. Three accounts are configured via mu4e contexts: gregnewman.org (personal Gmail
  with custom domain), NANOG (work Gmail), and Apple (iCloud). Outgoing mail is sent via
  msmtp, which reads credentials from =~/.authinfo.gpg= and selects the correct SMTP server
  based on the From address. Run =M-x mu4e= to open; =C-c C-c= to send a composed message.

  #+BEGIN_SRC emacs-lisp
    (use-package mu4e
      ;; mu4e ships with the mu binary installed via Homebrew, not from MELPA.
      ;; :straight nil tells straight.el not to manage it; load-path is resolved
      ;; dynamically from `brew --prefix mu` so it works regardless of Homebrew prefix
      ;; (Intel /usr/local vs Apple Silicon /opt/homebrew).
      :straight nil
      :load-path (lambda ()
                   (list (expand-file-name
                          "share/emacs/site-lisp/mu/mu4e"
                          (string-trim (shell-command-to-string "brew --prefix mu")))))
      :commands mu4e
      :config

      ;; ── Faces ─────────────────────────────────────────────────────────────
      ;; Flagged messages get a warm orange to stand out; replied messages get
      ;; a muted green so you can see at a glance what's been handled.
      (custom-set-faces
       '(mu4e-flagged-face  ((t (:foreground "#FF9500" :weight bold))))
       '(mu4e-replied-face  ((t (:foreground "#5AF78E" :weight normal)))))

      ;; Headers list is denser at one step smaller than the default font size.
      (add-hook 'mu4e-headers-mode-hook
                (lambda () (text-scale-set -1)))

      ;; mu ships its own Info manual; add it to the info path so C-h i can find it.
      ...
      (setq mu4e-maildir "~/.mailbox"
            ;; Drive sync from mu4e rather than relying solely on the launchd service.
            ;; mu4e runs mbsync then re-indexes automatically every 5 minutes.
            mu4e-get-mail-command "mbsync -c ~/.config/isync/config -a"
      ...
      ;; macOS's built-in smtpmail has TLS issues; msmtp is more reliable and
      ;; selects the correct SMTP account based on the envelope From address,
      ;; so multi-account sending Just Works without extra configuration.
      (setq message-send-mail-function 'message-send-mail-with-sendmail
            sendmail-program "/opt/homebrew/bin/msmtp"
            ;; --read-envelope-from tells msmtp to pick the account from the From header.
            message-sendmail-extra-arguments '("--read-envelope-from")
            ;; Prevent message.el from adding a redundant -f flag that confuses msmtp.
            message-sendmail-f-is-evil t)

      ;; ── Contexts ──────────────────────────────────────────────────────────
      ;; Each context sets the From address, full name, and folder paths for one
      ;; account. The match-func fires on every message mu4e displays; the first
      ;; context whose match-func returns non-nil becomes active. Matching is done
      ;; on the maildir path prefix, which is unique per account.
      (setq mu4e-contexts
            (list
             (make-mu4e-context
              :name "gregnewman.org"
              :match-func (lambda (msg)
                            (when msg
                              (string-prefix-p "/gregnewman.org"
                                               (mu4e-message-field msg :maildir))))
              :vars '((user-mail-address  . "greg@gregnewman.org")
                      (user-full-name     . "Greg Newman")
                      (mu4e-sent-folder   . "/gregnewman.org/[Gmail]/Sent Mail")
                      (mu4e-drafts-folder . "/gregnewman.org/[Gmail]/Drafts")
                      (mu4e-trash-folder  . "/gregnewman.org/[Gmail]/Trash")
                      (mu4e-refile-folder . "/gregnewman.org/[Gmail]/Archive")))

             (make-mu4e-context
              :name "NANOG"
              :match-func (lambda (msg)
                            (when msg
                              (string-prefix-p "/NANOG"
                                               (mu4e-message-field msg :maildir))))
              :vars '((user-mail-address  . "gnewman@nanog.org")
                      (user-full-name     . "Greg Newman")
                      (mu4e-sent-folder   . "/NANOG/[Gmail]/Sent Mail")
                      (mu4e-drafts-folder . "/NANOG/[Gmail]/Drafts")
                      (mu4e-trash-folder  . "/NANOG/[Gmail]/Trash")
                      (mu4e-refile-folder . "/NANOG/[Gmail]/Archive")))

             (make-mu4e-context
              :name "Apple"
              :match-func (lambda (msg)
                            (when msg
                              (string-prefix-p "/Apple"
                                               (mu4e-message-field msg :maildir))))
              :vars '((user-mail-address  . "newman.greg@me.com")
                      (user-full-name     . "Greg Newman")
      ...
      ;; pick-first: auto-select the matching context without prompting.
      ;; ask-if-none: only ask which context to use when composing a new me
ssage
      ;; if none can be inferred (e.g. composing from the main screen).
      (setq mu4e-context-policy 'pick-first
            mu4e-compose-context-policy 'ask-if-none)

      ;; ── Maildir shortcuts ─────────────────────────────────────────────────
      ;; Press j in the headers view to jump to a shortcut folder.
      (setq mu4e-maildir-shortcuts
            '((:maildir "/gregnewman.org/INBOX" :key ?g :name "gregnewman.org")
              (:maildir "/NANOG/INBOX"           :key ?n :name "NANOG")
              (:maildir "/Apple/INBOX"           :key ?a :name "Apple"))))

      ;; ── Header icons ──────────────────────────────────────────────────────
      ;; Replace mu4e's default ASCII flags with emoji for a cleaner headers view.
      ;; Each entry is (ascii-fallback . emoji-string). The seen-mark is blank
      ;; intentionally - read messages shouldn't draw the eye.
      ...
      ;; ── Bookmarks ─────────────────────────────────────────────────────────
      ;; mu4e-headers-include-related is disabled globally. When enabled, a date
      ;; query like "date:today..now" pulls in every message in any thread that
      ;; received a message today - including hundreds of old unread messages from
      ;; mailing lists archived months ago. Disabling it means queries return only
      ;; messages that literally match the query. Toggle per-search with W.
      (setq mu4e-headers-include-related nil)

      ;; Custom bookmarks replace the defaults. "Unread (INBOX)" scopes to only
      ;; the three INBOX folders so mailing list noise in Archive doesn't inflate
      ;; the unread count.
      (setq mu4e-bookmarks
            '((:name "Unread (INBOX)"
               :query "flag:unread AND NOT flag:trashed AND (maildir:/gregnewman.org/INBOX OR maildir:/NANOG/INBOX OR maildir:/Apple/INBOX)"
               :key ?u)
      ...
      ;; ── Calendar invites ──────────────────────────────────────────────────
      ;; gnus-icalendar is bundled with Emacs and handles .ics attachments in
      ;; mu4e message view. goto-addr activates clickable links in the view buffer.
      (require 'goto-addr)
      (require 'gnus-icalendar)

    ;; ── HTML rendering via xwidgets webkit ────────────────────────────────
    ;; mu4e's built-in shr renderer produces unreadable output for HTML-heavy
    ;; mail (Google Docs notifications, newsletters, etc.). mu4e-views replaces
    ;; the message view with a proper webkit browser via xwidgets. The default
    ;; method is "html" (webkit); press v in the headers view to pick a different
    ;; renderer for the current message. M-n/M-p scroll the message view without
    ;; leaving the headers buffer.
    (use-package xwidgets-reuse
      :straight (:type git :host github :repo "lordpretzel/xwidgets-reuse"))

    (use-package mu4e-views
      :straight (:type git :host github :repo "lordpretzel/mu4e-views")
      :after mu4e
      :defer nil
      ...
      ;; Keep the message view in place when navigating with M-n/M-p rather than
      ;; jumping back to the top of the message on each cursor move.
