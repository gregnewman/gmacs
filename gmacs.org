...
* Tasks I plan to complete regarding this config [80%]
- [X] Fix tangling config.  When I save it tangles the config then kills the buffer.
- [X] Change fonts
- [X] Change line height/spacing
- [X] I would love to have highlighting for css colors. I think i had this once before.
...
* Font handling
  #+BEGIN_SRC emacs-lisp
    (set-face-attribute 'default nil :font "Monaspace Neon" :weight 'light :height 140)
    (set-face-attribute 'fixed-pitch nil :font "Monaspace Neon" :weight 'light :height 130)
...
* One liners preferences
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (add-to-list 'auto-mode-alist '("\\.astro\\'" . javascript-mode))
...
#+BEGIN_SRC  emacs-list
  ...
  (add-to-list 'eglot-server-programs '((python-ts-mode python-mode) "python"))
#+end_src
...
#+END_SRC
* Spacious Padding
Trying this little [[https://git.sr.ht/~protesilaos/spacious-padding][padding package by Prot]].
#+BEGIN_SRC emacs-lisp
